...
namespace Savanna::Core::Tests
{
    ...
    protected:
        ...
        void CallDeallocate(FreeListAllocator& allocator, void* const ptr, const size_t alignment)
        {
    ...
    {
        void* ptr = nullptr;
        CallAllocate(m_FreeListAllocator, 1, 1, &ptr);
    ...
    DECLARE_FREE_LIST_TEST_F(EnsureInvalidAllocationReturnsNullptrOnEmptyAllocator)
    {
        ...
        size_t alignment = 8;
        size_t size = 2048;
        CallAllocate(m_FreeListAllocator, size, alignment, &result);
        EXPECT_EQ(result, nullptr);
    }

    DECLARE_FREE_LIST_TEST_F(CheckIfAlignedAllocatedPtrIsProperlyOffset)
    {
        void* result = nullptr;
        size_t alignment = 8;
        size_t size = 8;
